...
class HRDAEncoderDecoder(EncoderDecoder):
    ...
    def encode_decode_with_scale(self, img, img_metas, up=True):
        ...
        for i, s in enumerate(self.scales):
            ...
            if self.decode_head.debug:
                self.decode_head.debug_output[f'Img {i} Scale {s}'] = \
                    scaled_img.detach()
        out1 = self._decode_head_forward_test(mres_feats, img_metas)
        if up is True:
            out2 = resize(
                input=out1,
                size=img.shape[2:],
    ...
    def encode_decode_with_proj(self, img, img_metas, up=True):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        with torch.no_grad():
            mres_feats = []
        ...
        x = mres_feats[0]
        _, _, h, w = x[0].size()
        feat1 = x[0]  # 大小为128，128的
        feat2 = F.interpolate(x[1], size=(h, w), mode="bilinear", align_corners=False)
        feat3 = F.interpolate(x[2], size=(h, w), mode="bilinear", align_corners=False)
        feat4 = F.interpolate(x[3], size=(h, w), mode="bilinear", align_corners=False)
    ...
    def forward_train_source(self,
        ...
        feat1 = x_[0]  # 大小为128，128的
        feat2 = F.interpolate(x_[1], size=(h, w), mode="bilinear", align_corners=False)
        feat3 = F.interpolate(x_[2], size=(h, w), mode="bilinear", align_corners=False)
        feat4 = F.interpolate(x_[3], size=(h, w), mode="bilinear", align_corners=False)
        feats = torch.cat([feat1, feat2, feat3, feat4], 1)
        emb = self.proj_head(feats)
        loss_decode = self._decode_head_forward_train_source(mres_feats,
                                                             emb, img_metas, gt_semantic_seg, seg_weight, prepare_dict)
    ...
    def forward_train_mix(self,
        ...
        for i, s in enumerate(self.scales):
            ...
            if return_feat and s == self.feature_scale:
                losses['features'] = mres_feats[i]
                break
        loss_decode = self._decode_head_forward_train_mix(mres_feats,
                                                          img_metas,
                                                          gt_semantic_seg,
                                                          seg_weight)
        losses.update(loss_decode)
